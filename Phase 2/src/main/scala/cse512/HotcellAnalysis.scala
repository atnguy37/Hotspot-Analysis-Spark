...
//  sumOfSquares.createOrReplaceTempView("sumOfSquares")
  ...
  //spark.udf.register("adjacentCells", (inputX: Int, inputY: Int, inputZ: Int, minX: Int, maxX: Int, minY: Int, maxY: Int, minZ: Int, maxZ: Int) => ((HotcellUtils.calculateAdjacentCells(inputX, inputY, $
  ...
  val adjacentCells = spark.sql("select sch1.x as x, sch1.y as y, sch1.z as z,"
      ...
      + "and (sch2.z != sch1.z and sch2.y != sch1.y and sch2.x != sch1.x) "
  ...
  pickupInfo = spark.sql("select zScore(adjacentCellCount, sumHotCells, "+ numCells + ", x, y, z," + mean + ", " + standardDeviation + ") as getisOrdStatistic, x, y, z from adjacentCells order by getisOrdStatistic DESC, z DESC")
