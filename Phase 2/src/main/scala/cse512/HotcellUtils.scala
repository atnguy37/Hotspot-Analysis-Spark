...
object HotcellUtils {
  ...
  def calculateZScore(adjacentCellCount: Int, sumHotCells: Int, numCells: Int, x: Int, y: Int, z: Int, mean: Double, standardDeviation: Double): Double =
  {
    ...
    val denominator = standardDeviation * math.sqrt(((numCells.toDouble * adjacentCellCount.toDouble) - (adjacentCellCount * adjacentCellCount).toDouble) / (numCells - 1).toDouble).toDouble
